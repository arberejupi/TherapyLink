...
const ClinicDetails = () => {
  ...
  useEffect(() => {
    const fetchClinicDetails = async () => {
      try {
        const response = await fetch(`http://localhost:5179/api/Clinic/GetClinicById=${clinicId}`);
        if (response.ok) {
    ...
    const fetchClinicDoctors = async () => {
      try {
        const response = await fetch(`http://localhost:5179/api/Doctor/clinic/${clinicId}`);
        if (response.ok) {
          const data = await response.json();

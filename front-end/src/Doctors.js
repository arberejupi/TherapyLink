...
const Doctors = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        const response = await fetch('http://localhost:7207/api/Doctor/GetAllDoctors');
        if (response.ok) {
  ...
  const fetchClinicName = async (clinicId) => {
    try {
      const response = await fetch(`http://localhost:7207/api/Clinic/GetClinicById?clinicId=${clinicId}`);
      if (response.ok) {
        const data = await response.json();
        return data.name;
      } else {
